...
exports.run = (client, message) => {
  if (!message.member.hasPermission("ADMINISTRATOR")) return message.reply(`Bu komutu kullanabilmek için **Yönetici** iznine sahip olmalısın!`);
    ...
    .addField(`Doğru Kullanım:`, `${ayarlar.prefix}caps-lock-engelle aç veya kapat`)
    if(secenekler.length < 1) return message.channel.send(errembed);
    //if(secenekler === "aç" || "kapat") return message.channel.send(errembed);
      if(secenekler.length < 1) return message.reply("CapsLock Engelleme Açmak İçin `&caps-lock-engelle aç` kapatmak için `&caps-lock-engelle kapat`").then(m => m.delete(10000));
