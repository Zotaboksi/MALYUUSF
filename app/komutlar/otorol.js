...
exports.run = async (bot, message, args) =>
{
  ...
  if (!mentionedChannel && args[0] !== "sıfırla") return message.channel.send("**Otomatik Rol Sistemi Bilgi** \n\n Kullanım: `gr!otorol @Verilecek Rol #Otorol Mesaj Kanalı` ");
  if (message.guild.member(message.author.id).hasPermission(0x8))
    
    {
      var mentionedRole = message.mentions.roles.first();
      if (!mentionedRole) return message.channel.send("**Doğru Kullanım = gr!otorol @<roladı> #<metinkanalı>**".then(msg => msg.delete(5000)));
  ...
  profil[message.guild.id].kanal = mentionedChannel.id
    ...
    fs.writeFile("./otorol.json", JSON.stringify(profil), (err) => {
        ...
        message.channel.send(":onay11: ◆ **Otorol Ayarlandı** ◆ \n\n┏╋━━━━◥◣◆◢◤━━━━╋┓\n\nOtorol bu sunucuda aktif edildi. \n\nKapatmak için `c!otorol-kapat` yazabilirsin.\n\n┗╋━━━━◥◣◆◢◤━━━━╋┛")
